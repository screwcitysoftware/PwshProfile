function Enable-Less {
    ...
    .PARAMETER ReplaceMore
        Set $env:PAGER to 'less' — routing PowerShell's `help`, bat, git, delta and gh through it — and
        alias more -> less globally. Off by default, leaving $env:PAGER and more untouched.

    .EXAMPLE
        Enable-Less

        Installs less if needed and sets $env:LESS, leaving more.com as PowerShell's help pager. bat
        auto-detects less and pages with color.
    ...
    .NOTES
        ...
        but honors $env:PAGER, which is what -ReplaceMore relies on. The more -> less alias alone does
        not redirect `help`, since `help` invokes the literal string more.com.
    #>
    [CmdletBinding()]
    param(
        [Parameter()]
