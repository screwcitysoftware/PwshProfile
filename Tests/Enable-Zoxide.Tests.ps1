...
Describe 'Enable-Zoxide' {
    BeforeEach {
        ...
        # Snapshot (not just clear) any pre-existing hook globals. If a real profile's Enable-Zoxide was
        # already active before the suite ran (this module doubles as the author's own profile),
        # $script:savedLoc above just captured a REAL hook closure that itself reads these globals --
        # AfterEach must put them back exactly, not merely delete them, or restoring that real hook
        # leaves it referencing variables that no longer exist, so the very next Set-Location anywhere
        # later in the suite throws under StrictMode.
        $script:savedZoxideGlobals = @{}
        foreach ($name in '__zoxide_loc_hooked', '__zoxide_loc_base') {
            $existing = Get-Variable -Name $name -Scope Global -ErrorAction SilentlyContinue
            if ($existing) { $script:savedZoxideGlobals[$name] = $existing.Value }
        }
    ...
    AfterEach {
        ...
        foreach ($name in '__zoxide_loc_hooked', '__zoxide_loc_base') {
            if ($script:savedZoxideGlobals.ContainsKey($name)) {
                Set-Variable -Name $name -Value $script:savedZoxideGlobals[$name] -Scope Global
            }
            else {
